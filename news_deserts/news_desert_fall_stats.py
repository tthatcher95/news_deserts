...
def news_deserts_csv_df():
    ...
    basepath = "/projects/canis/news_deserts/twitter/data/"
    # fileName = "/Users/tdt62/Desktop/october_files.txt"
    filename = "/projects/canis/news_deserts/twitter/october_final.txt"
...
def hashtags(df, top_val):
    ...
    return top_list, sorted_news_list, hashtag_dict
...
#         filtered_urls = df[(df['URLs'] != '[]')]
    ...
    # return top_list, sorted_domain_list, domain_dict
    return domain_dict
...
def get_top_noun_phrases(df, top_val):
    ...
    # Gets the sorted news_list in descending order
    sorted_phrase_list = (list(sorted(phrase_dict.items(), key=itemgetter(1), reverse=True)))

    # Gets the top 20 results of the sorted list
    sorted_phrase_list = sorted_phrase_list[0:top_val]

    # Stores all the names in a list, so we can read correctky
    for item in sorted_phrase_list:
        top_list.append(item[0])

    return top_list, sorted_phrase_list
...
def plot_top_metrics(top_list, top_val=5, metric="Hashtag", with_grid=False, save_image=True, imagepath="/Users/tdt62/Desktop/GraduateResearch/test_data/"):
    x, y = zip(*top_list[:top_val])
    fig, ax = plt.subplots()
    ax.bar(x,y,align='center', width=0.5, color = 'b')
    ax.set_xticks(x)
    ax.set_xticklabels(x)
    ax.set_xlabel("This graph shows the Top {}".format(metric))
    ax.set_ylabel("# Times {} Used".format(metric))
    ax.grid(with_grid)
    if save_image:
        plt.savefig(imagepath + metric + ".png")
    else:
        plt.show()
...
if __name__== "__main__":
    ...
    print("Total Number of Entries: {}".format(len(news_df)))
    print("Number of Entries with Links: {}".format(len(filtered_urls)))
    ...
    # plot_top_metrics(sorted_top_news_list, metric="News Domains", imagepath="/projects/canis/scripts/graduate_research/news_deserts_stats/entire_election/plots/")

    # Gets top 20 domains
    # domain_dict = get_top_domains(news_df, 20)
    # # plot_top_metrics(sorted_top_domain_list, metric="Domains", imagepath="/projects/canis/scripts/graduate_research/news_deserts_stats/entire_election/plots/")
    # make_csv(domain_dict, "/projects/canis/scripts/graduate_research/news_deserts_stats/entire_election/october/files/", "top_domains_unshort.csv", headers=['Domain', 'Count'])
    ...
    # top_hashtags, hashtag_sorted_list, hash_dict = hashtags(news_df, 20)
    # # plot_top_metrics(hashtag_sorted_list, metric="Hashtags", imagepath="/projects/canis/scripts/graduate_research/news_deserts_stats/entire_election/plots/")
    # make_csv(hash_dict, "/projects/canis/scripts/graduate_research/news_deserts_stats/entire_election/october/files/", "top_hashtags.csv", headers=['Hashtag', 'Count'])
    #
    # # Get the top 20 user mentions
    # top_users_list, sorted_news_list = top_user_mentions(news_df, 20)
    #
    # # Get the top 10 noun phrases
    # top_phrases, sorted_phrases = get_top_noun_phrases(news_df, 10)
    # plot_top_metrics(sorted_phrases, metric="Noun Phrases", imagepath="/projects/canis/scripts/graduate_research/news_deserts_stats/entire_election/plots/")
    #
    # # Prints each value
